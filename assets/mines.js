...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-361d67f8";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-361d67f8">'+'<span class="tile-face-361d67f8 tile-front-361d67f8"></span>'+'<span class="tile-face-361d67f8 tile-back-361d67f8"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-361d67f8")){return;}
var back=tile.querySelector(".tile-back-361d67f8");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-361d67f8");if(isMine){tile.classList.add("boom-361d67f8");}
if(dim){tile.classList.add("dimmed-361d67f8");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-361d67f8")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-361d67f8");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-361d67f8")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-361d67f8");void panelEl.offsetWidth;panelEl.classList.add("jolt-361d67f8");}
endRound();}
